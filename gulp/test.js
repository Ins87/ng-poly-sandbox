...
module.exports = function (gulp, $, config) {
  ...
  gulp.task('copyTestDirectiveTemplates', ['markup'], function () {
    // always copy templates to testBuild directory
    var stream = $.streamqueue({objectMode: true});

    stream.queue(gulp.src([config.buildDirectiveTemplateFiles]).pipe($.cached('copyTestDirectiveTemplates')));
  ...
  gulp.task('buildTests', ['lint'], function () {
    return gulp.src([config.unitTestFiles])
      .pipe($.cached('buildTests'))
      .pipe(gulp.dest(config.buildUnitTestsDir));
  });

  // inject scripts in karma.config.js
  gulp.task('karmaFiles', ['buildTests', 'copyTestDirectiveTemplates'], function () {
    var stream = $.streamqueue({objectMode: true});
